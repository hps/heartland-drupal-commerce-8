      ...
      $('.heartland-form', context).once('heartland-processed').each(function () {
        ...
        var hps = new Heartland.HPS({
          ...
          onTokenSuccess: function (resp) {
            $('#token_value', form).val(resp.token_value);
            $('#card_type', form).val(resp.card_type);
            $('#last_four', form).val(resp.last_four);
            $('#exp_month', form).val(resp.exp_month);
            $('#exp_year', form).val(resp.exp_year);
            $('#token_expire', form).val(resp.token_expire);
            form.get(0).submit();
          },
